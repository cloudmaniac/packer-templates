/*
    DESCRIPTION: Ubuntu 20.04 LTS (Focal Fossa) vSphere template.
...
source "vsphere-iso" "ubuntu" {
  ...
  boot_command         = ["<esc><wait><esc><wait><f6><wait><esc><wait>", "autoinstall ds=nocloud-net;s=http://{{ .HTTPIP }}:{{ .HTTPPort }}/ ", "--- <enter>"]
  boot_wait            = "5s"
